  ...
  if (user) {
    await prisma.loginAttempt.create({
      data: {
        ...
        username: user.username, // Set the username obtained from the User record
        ipAddress: ipAddress,
        // Other fields...
      },
    });
...
if (!passwordMatch) { return res.status(401).json({ message: 'Invalid password or email' }); }
...
res.cookie('RefreshJWTTokenHttpOnly', refreshToken, {
  expires: new Date(Date.now() + 16 * 60 * 60 * 999), // 16 hours in milliseconds
  httpOnly: true,
  secure: process.env.NODE_ENV === 'production', // Set to true in production
  sameSite: 'strict'
}); 
